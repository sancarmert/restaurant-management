...
func CreateInvoices() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		invoice.Created_at, _ = time.Parse(time.RFC1123, time.Now().Format(time.RFC3339))
		invoice.Updated_at, _ = time.Parse(time.RFC1123, time.Now().Format(time.RFC3339))
		...
		if insertErr != nil {
			msg := fmt.Sprint("invoice item was not created")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func UpdateInvoices() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprint("invoice item failed")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
